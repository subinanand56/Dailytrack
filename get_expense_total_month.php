...
if (isset($_GET['branch'])) {
    ...
    if ($selectedBranch === 'All') {
        ...
        $sql = "SELECT SUM(price) AS total_amount FROM expense WHERE MONTH(date) = '$currentMonth' AND YEAR(date) = '$currentYear'";
    } else {
        // Fetch total sum of 'price' column for the current month for the selected branch
        $sql = "SELECT SUM(price) AS total_amount FROM expense WHERE MONTH(date) = '$currentMonth' AND YEAR(date) = '$currentYear' AND branch = '$selectedBranch'";
    }
