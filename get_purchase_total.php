...
if (isset($_GET['branch'])) {
    ...
    if ($selectedBranch === 'All') {
        ...
        $sql = "SELECT SUM(price) AS total_amount FROM purchase WHERE DATE(date) = '$todayDate' AND accepted='1'";
    } else {
        // Fetch total sum of 'price' column for today for the selected branch
        $sql = "SELECT SUM(price) AS total_amount FROM purchase WHERE DATE(date) = '$todayDate' AND branch = '$selectedBranch' AND accepted='1'";
    }
