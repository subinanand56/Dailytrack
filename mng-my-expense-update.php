    ...
    if (!empty($_POST['expense_id']) && !empty($_POST['amount']) && !empty($_POST['item']) && !empty($_POST['date'])) {
        ...
        if ($result) {
            ...
            if ($review_status == '0') {
                ...
                $qry = "UPDATE `expense` SET `price`='$amount', `item`='$item', `date`='$date', `Eid`='$Eid' WHERE `id`='$expense_id'";
                $run = mysqli_query($conn, $qry);

                if ($run) {
                    echo "<script type='text/javascript'>alert('Expense Updated Successfully'); window.location.href='mang-my-expense.php';</script>";
                } else {
                    echo "Failed to update expense, try again";
                }
            } else {
                echo "<script type='text/javascript'>alert('Expense cannot be updated as it has been reviewed by the manager'); window.location.href='mang-my-expense.php';</script>";
            }
        } else {
